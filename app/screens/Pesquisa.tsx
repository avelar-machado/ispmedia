...
const SearchComponent: React.FC = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        const [albumsResponse, artistsResponse, imagesResponse, usersResponse] = await Promise.all([
          axios.get<Album[]>('http://192.168.1.109:3000/admin/albuns'),
          axios.get<Artista[]>('http://192.168.1.109:3000/artistas'),
          axios.get<Imagem[]>('http://192.168.1.109:3000/images'),
          axios.get<User[]>('http://192.168.1.109:3000/users')
        ]);
  ...
  const getImageUrl = (imageId: number | null) => {
    ...
    return image ? `http://192.168.1.109:3000/api/upload/image/${nameUser?.username}/${image.nome_ficheiro}` : 'http://192.168.1.109:3000/api/upload/image/avelarmanuel/default.jpg';
  };
